...
class GardenInfoView: UIView {
    ...
    func configure(seedCount: Int, fruitCount: Int) {
        seedItem.configure(count: seedCount)
        fruitItem.configure(count: fruitCount)
    }
}
